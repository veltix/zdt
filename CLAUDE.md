...
The Laravel Zero guidelines are specifically curated by Laravel Zero maintainers for this application. These guidelines should be followed closely to enhance the user's satisfaction building Laravel Zero applications.
...
- Final classes and readonly where possible.
...
## Do Things the Laravel Zero Way

- Use `php zdt make:` commands to create new files (i.e. commands, tests).
...
- When creating tests, make use of `php zdt make:test [options] <name>` to create a feature test, and pass `--unit` to create a unit test. Most tests should be feature tests.


=== laravel-zero/v12 rules ===
...
- There is `app/Commands` and `app/Providers` folder.
- The `app/Commands` folder should contain your application's Artisan commands. By default, it provides `app/Commands/InspiringCommand.php`
...
<code-snippet name="Service Provider Example" lang="php">
...
<code-snippet name="Service Provider Contract Example" lang="php">
...
### Pest Tests
- All tests must be written using Pest. Use `php zdt make:test <name>`.
...
- To run all tests: `php zdt test`.
- To run all tests in a file: `php zdt test tests/Feature/ExampleTest.php`.
- To filter on a particular test name: `php zdt test --filter=testName` (recommended after making a change to a related file).
